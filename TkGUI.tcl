...
frame .right -bd 1 -borderwidth 2 -height 385 -relief groove -width 340
	...
	bind .right.cbpackage_server <Button-3> { EditCombo .right.cbpackage_server includes/package_servers.txt }
	...
	.right.cbautoupdate list insert end 	"slapt-get" \
						"swaret" \
						"none"	
	...
	set TAG "mini-tag"
        ...
        .right.cbsecure list insert end     "yes" \
                                            "no"
	...
	.right.cbhwclockzone list insert end  	"localtime" \
        	                      		"utc"
	...
	combobox .right.cbautostart \
        ...
        .right.cbautostart list insert end  "yes" \
                                            "no"
...
frame .down -bd 1 -borderwidth 2 -height 120 -relief groove -width 300
	...
	combobox .down.cbpart-fs\
        ...
        listbox .down.lista -width 44 -height 10 -foreground black -background lightblue -font fixed -selectbackground "#00ccff" -selectforeground black
	...
	bind .down.lista <Button-1> { if { [%W get [%W nearest %y]] != "" && $EDITING != "True" } { .down.bedit-part configure -state normal }}

        button  .down.bpart-active -width 15 \
                -text {Add partition    } -command { Parser }

        button  .down.bedit-part -width 15 -state disabled -text {Edit partition    } -command Edit
        button  .down.bdelete-part -width 15 -state disabled -text {Delete last partition} -command { ClearPartList LAST }
	button 	.down.bclearpartitions -width 15 -state disabled -text {Delete all Partitions} -command { ClearPartList ALL }
...
frame .configcmd -bd 1 -borderwidth 2 -height 100 -relief groove -width 300
	...
	button .configcmd.bloadconfig -width 19 -text {Load Config File} -command { LoadConfigFile $CONFIGFILE}
