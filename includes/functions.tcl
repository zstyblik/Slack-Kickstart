...
foreach control { .left.eip .left.cbnetmask .left.edefaultgw .left.edomain .left.edns1 .left.edns2 } {
...
proc LoadComboCfg {COMBOBOX PATH} {
	...
	# vonatar[04/06/03]
          ...
          foreach u [ exec ls $PATH ] \
                { if { [ regexp {\-tag|\.cfg} $u ]} { $COMBOBOX list insert end  $u } }
...
set Kernel [ catch { exec file $KERNEL | grep "x86 boot sector" }  ]
...
proc LoadKernel {COMBOBOX } {
       	...
       	set KernelLIst [ catch { exec find kernels -type f | xargs file  | grep "x86 boot sector" }  ]
       	set u 0
