...
if [ $# -eq 3 ] ;then
    ...
    PACKAGE_SERVER=`cat Config-Files/$HOST.cfg | grep PACKAGE_SERVER | sed s/PACKAGE_SERVER=//g`
	...
	for PACKAGE in `cat Taglists/$TAG | grep -v "#" | cut -d ":" -f 1`
	do
		...
		echo
		echo -n "Copying packages for diskset [ $DISKSET ] "
            fi
		echo -n "."
        	cp $SLACKCD/slackware/$PACKAGE*.tgz $ISODIR/slackware/$DISKSET/
